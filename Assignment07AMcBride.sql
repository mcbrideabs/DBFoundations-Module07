...
-- 2024-08-11, Abigail McBride, Modified File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AMcBride')
	 Begin 
	  Alter Database [Assignment07DB_AMcBride
] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AMcBride
;
	 End
	Create Database Assignment07DB_AMcBride;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AMcBride;
...
Select 
  ProductName
, FORMAT(InventoryDate, 'Y') as [InventoryDate]
, Count
FROM vProducts
JOIN vInventories
...
-- need to use different format to manipulate date, add proper name for count
Select 
  ProductName
...
--InventoryDate not sorting properly
Select 
  ProductName
, [InventoryDate] = DateName(MM, InventoryDate) + ', ' + DateName(YY, InventoryDate)
, [InventoryCount] = Count
...
go
---need to simplify since using previous view
create function fProductInventoriesWithPreviousMonthCountsWithKPIs (@KPIValue Int)
RETURNS table 
AS
	return select 
...
[Google Homepage] (https://google.com "Google's Homepage")

[Github Webpage Code CheatSheet] (https://mcbrideabs.github.io/ITFnd100-Mod07/)
GO
